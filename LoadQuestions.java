...
public class LoadQuestions implements Runnable {
	...
	public void run() {
		...
		while (fileReader.hasNext()) {
			...
			// slipt 100 quesions into 10 different topics using charAt() to check if the
			// first character of the line is ":" (Thau)
			if (holder.charAt(0) == ':' || holder.charAt(1) == ':') {
				// Checks to make sure its not on the first rotation before offering the packet
				// of questions.
				if (list != null) {
					que.offer(list);
				}

...
public class ChartPanel extends JPanel {
	...
	public ChartPanel(int[] c) {
		...
		// int array c takes the answersCorrect from each category to be used for the
		// size of the bar as well as summed up to give totalanswersCorrect for the
		// title.
		// creates String array with values of the categories (Arrays, inheritance,
		// interface, etc) which will be used to draw under the bars
		// int totalQuestionsAsked is set to 30 questions
		// int totalAnswersCorrect which will be summed from for each loop using the int
		// answersCorrect
		// title will display the answers correct, total questions, percentage from
		// answers correct over total questions
		names = new String[10];
		names[0] = "Arrays";
		names[1] = "Inheritance";
		names[2] = "Interface";
		...
		// for each loop to add all of the answers correct together which will be used
		// in the title
		int totalAnswersCorrect = 0;

		for (int i : answersCorrect) {
			totalAnswersCorrect += i;
		}

		double correctPercentage = ((double) totalAnswersCorrect / totalQuestionsAsked) * 100;

		title = String.format("Answers Correct: %d / %d  |  %.2f%% Correct", totalAnswersCorrect, totalQuestionsAsked,
				correctPercentage);
	...
	public void paintComponent(Graphics g) {
		...
		// for loop to draw a bar with size based on answersCorrect[i] and fill the bars
		// with random colors
		// also draw names underneath bars with the categories
		// label each bar with the number from answersCorrect[i] (answers correct in
		// that category)
		for (int i = 0; i < answersCorrect.length; i++) {
	...
	public static void main(String[] argv) {
		...
		// test method as the Final project will just use the constructor to create the
		// graph.
